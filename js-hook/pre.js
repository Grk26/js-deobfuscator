...
var emu_plugins = new PluginArray();
emu_plugins.push(new Plugin('getPlusPlus for Adobe 16263','np_gp.dll','getplusplusadobe16263'));
emu_plugins.push(new Plugin('Google Talk Plugin','npgoogletalk.dll','Version 1,0,21,0'));
emu_plugins.push(new Plugin('Adobe Acrobat','nppdf32.dll','Adobe PDF Plug-In For Firefox and Netscape'));
emu_plugins.push(new Plugin('Mozilla Default Plug-in','npnul32.dll','Default Plug-in'));
emu_plugins.push(new Plugin('Microsoft Office 2003','NPOFFICE.DLL','Office Plugin for Netscape Navigator'));
emu_plugins.push(new Plugin('Google Update','npGoogleOneClick8.dll','Google Update'));
emu_plugins.push(new Plugin('Shockwave Flash','NPSWF32.dll','Shockwave Flash 10.0 r32'));
emu_plugins.push(new Plugin('Silverlight Plug-In','npctrl.dll','3.0.50106.0'));
emu_plugins.push(new Plugin('Microsoft Office Live Plug-in for Firefox','npOLW.dll','Office Live Update v1.4'));
emu_plugins.push(new Plugin('Windows Live® Photo Gallery','NPWLPG.dll','NPWLPG'));
emu_plugins.push(new Plugin('Java Deployment Toolkit 6.0.140.8','npdeploytk.dll','NPRuntime Script Plug-in Library for Java(TM) Deploy'));
emu_plugins.push(new Plugin('Java(TM) Platform SE 6 U14','npjp2.dll','Next Generation Java Plug-in 1.6.0_14 for Mozilla browsers'));

function emu_navigator(){
    ...
    this.mimeTypes = {};
    this.plugins = emu_plugins;
}
var navigator= new emu_navigator();
var screen = {};

var app = {
	viewerVersion:Number(8.0),
    viewerType:String('Reader'),
	setTimeOut:function(txt,wait){ eval(txt); print ("; //called setTimeOut with "+txt + ', ' + wait);},
	clearTimeOut:function(a){},
	eval:function(a){eval(a);},
    alert:function(a){ print ("/*** app.alert " + a + "*/"); },
};

function emu_activex(){
	zzzactivex.push(this);
	this.donePath = 0;
	this.SaveToFile = this.savetofile = function (txt){ print("//save " + txt) }
	this.close = function(){ }
	...
	this.Write = this.write = function (txt){ print("//write " + txt) }
...
var the_activex = new emu_activex();

function emu_element(){
...
function emu_value(i){
    this.value = valzzz[i];
    this.innerText = txtzzz[i];
    this.innerHTML = txtzzz[i];
    this.style = style;
    this.object = new emu_activex();
...
var document= 	{ 
		...
		createElement : function(ele){print ('//called CreateElement ' +ele); elementn = elementn + 1; elements[elementn] = new emu_element(); return elements[elementn]},
		getElementById : function(i){ return new emu_value(idzzz.indexOf(i)); },
...
document.title = String('about');
document.location = String('http://localhost/');
document.body = new emu_element();
...
function emu_collab(){
	this.collectEmailInfo = function (txt){ print ("//called Collab.collectEmailInfo"); }
	this.getIcon = function (i){ print("//called collab.getIcon ") }
}
var Collab = new emu_collab();
...
app.plugIns.push(new PlugIn('SaveAsRTF','SaveAsRTF.api'));new 
...
var location = new emu_location('http://example.com','example.com'); 
